...
static void BC_Master()
{ // The head function of the master process.
...
static void BC_Worker()
{ // The head function of a worker process.
	bool exit;
	while (true)
	{
		exit = BC_WorkerMap();
...
{ // Forms an order and sends it to the worker processes to perform the Map function in the current iteration.
...
#pragma omp parallel for num_threads(PP_BSF_NUM_THREADS) schedule(static, chunk_size)
#else
#pragma omp parallel for schedule(static, chunk_size)
#endif // PP_BSF_NUM_THREADS
#endif // PP_BSF_OMP
	for (int i = offset; i < offset + sublistSize; i++)
	{
		int index = i - offset;
...
static void BC_Init(bool *success)
{ // Performs the memory allocation and the initialization of the skeleton data structures and variables.
	...
	{
		if (BD_rank == 0)
			cout << "Error: MPI_SIZE must be < Map List Size + 2 =" << BD_listSize + 2 << endl;
		MPI_Finalize();
		exit(1);
	};
	...
	else
	{
		...
		for (int rank = 0; rank < BD_numOfWorkers; rank++)
		{
			...
			// Inicializar BD_chunkSize para cada trabajador
			BD_chunkSize[rank] = (BD_sublistSize[rank] + PP_BSF_NUM_THREADS - 1) / PP_BSF_NUM_THREADS;
		}
	...
	if (PP_BSF_MAX_JOB_CASE > 0)
	{
		...
		if (BD_extendedReduceList_1 == NULL)
		{
			*success = false;
			return;
		};
	...
	if (PP_BSF_MAX_JOB_CASE > 1)
	{
		...
		if (BD_extendedReduceList_2 == NULL)
		{
			*success = false;
			return;
		};
	...
	if (PP_BSF_MAX_JOB_CASE > 2)
	{
		...
		if (BD_extendedReduceList_3 == NULL)
		{
			*success = false;
			return;
		};
...
static void BC_MpiRun()
{
	...
	{
		if (BD_rank == 0)
			cout << "Error: MPI_SIZE exceeded the maximum allowed value PP_BSF_MAX_MPI_SIZE = " << PP_BSF_MAX_MPI_SIZE << endl;
		MPI_Finalize();
		exit(1);
	}
	...
	{
		if (BD_rank == 0)
			cout << "Error: MPI_SIZE must be > 1" << endl;
		MPI_Finalize();
		exit(1);
	};
};