...
//----------------------------- User functions -----------------------------
static bool StopCond(PT_bsf_parameter_T *parameter)
{							// Calculates the stop condition
...
{ // Calculates the square of the vector norm
	double sum = 0;
	for (int j = 0; j < PP_N; j++)
		sum += x[j] * x[j];
	return sqrt(sum);
...
inline PT_float_T Alpha(int i, int j)
{ // Reduced matrix Alpha
	if (j == i)
		return 0;
	return -A(i, j) / A(i, i);
}

inline PT_float_T beta(int i)
{ // Reduced colunm beta
	return b(i) / A(i, i);
}